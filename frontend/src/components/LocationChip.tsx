...
export default function LocationChip({ location, size }: Props) {
    const { theme } = useThemeContext();

    const containerStyles: StyleProp<ViewStyle> = size === 'small' ?
    {
        borderRadius: 24,
        paddingVertical: 2,
        paddingRight: 4,
    ...
    const textStyles: StyleProp<TextStyle> = size === 'small' ? { fontSize: 10 } : {};
